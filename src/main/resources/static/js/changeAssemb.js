...
function updateLineData(data1){
    ...
    if(startHour == 0){
        minValue = 0;
        maxValue1 = Math.max(maxValue2, data1[data1.length-1]);
...
function changeData(data) {
   ...
   // console.log(uData);
    ...
    weldGas.innerText=parseFloat(data.weldGas).toFixed(2).toString()+"Nm³";
    //weldWire.innerText=parseFloat(data.weldWire).toFixed(2).toString()+"L";
    ...
    updateLineData(data.hourlyGasData);
...
setInterval(function(){
    $.ajax({
        url:"/data/S05Data?access_token="+text,
        type:"GET",
        success:(result)=>{
            changeData(result);
           // changeTick(result);
